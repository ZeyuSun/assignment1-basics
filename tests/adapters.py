...
def run_train_bpe(
    ...
    **kwargs,
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    # Initialize vocabulary: add single bytes (0-255) and special tokens
    ...
    # Pretokenization: split text into pre-tokens
    pretokens = pretokenize(text, special_tokens)
    pretoken_cnt = Counter()  # counting frequencies of pre-tokens
...
def initialize_vocab(special_tokens: list[str]) -> dict[int, bytes]:
    """Initialize vocabulary with single bytes and special tokens.

    Args:
        special_tokens (list[str]): A list of string special tokens to be added to the tokenizer vocabulary.
            These strings will never be split into multiple tokens.

    Returns:
        dict[int, bytes]: The initialized tokenizer vocabulary, a mapping from int (token ID in the vocabulary)
            to bytes (token bytes)
    """
    # Add special tokens
    vocab = {i: t.encode("utf-8") for i, t in enumerate(special_tokens)}

    # Add single bytes (0-255) if not already in vocab
    for i in range(256):
        b = bytes([i])  # chr(i).encode("utf-8")
...
def word2byte(word: str) -> tuple[bytes, ...]:
    ...
    NOTE:
    - bytes('こo!', 'utf-8') returns a single bytes object: b'\xe3\x81\x93o!'
    - tuple(bytes('こo!', 'utf-8')) returns individual integers, not bytes.
    - word2byte('こo!') returns (b'\xe3', b'\x81', b'\x93', b'o', b'!')
    """
    word_encoded = list(word.encode('UTF-8'))
    word_byte = [bytes([b]) for b in word_encoded]
